...
import com.sonicle.commons.db.TempTable;
import com.sonicle.commons.web.json.JsonResult;
import com.sonicle.commons.web.servlet.Crud;
...
public class MailService extends com.sonicle.webtop.core.sdk.Service {

	public final static Logger logger = getLogger(MailService.class);
    ...
    static {
        ...
        for(String fs: MailService.flagStrings) {
...
//      inlineableMimes.add("message/rfc822");
  ...
  public void initialize() {
    ...
    mprofile=new MailUserProfile(environment);
    //wta=env.getWebTopApp();
    //wtd=env.getWebTopDomain();
    //wts=env.getWebTopSession();
    ...
    FP.add("Message-ID");
    FP.add("X-Priority");
    if (profile.hasDocumentManagement()) FP.add("X-WT-Archived");
...
/*  public Vector getFoldersCache(boolean refresh) {
  ...
  public FolderCache[] getSharedFoldersCache() throws MessagingException {
      if (fcShared==null) {
        if (sharedPrefixes!=null) {
            ...
            for(int i=0;i<sf.length;++i) fcShared[i]=getFolderCache(sf[i]);
        }
      }
      return fcShared;
  }
    ...
    public boolean saveMailInDocumentsDrm(String path,String filename,String request_id,String generic_id){
        ...
        try{
            ...
            ex.printStackTrace();
                ...
                if (stmt!=null) try { stmt.close(); } catch(Exception exc) {}
                if (fi!=null) try { fi.close(); } catch(Exception exc) {}
                if (con!=null) try { con.close(); } catch(Exception exc) {}
        }
        return success;
    }
  ...
  public String getLastFolderName(String fullname) {
    ...
    if (lasttname.indexOf(folderSeparator)>=0) lasttname=lasttname.substring(lasttname.lastIndexOf(folderSeparator)+1);
    return lasttname;
  }
  ...
  public boolean deleteFolder(Folder folder) throws MessagingException {
      ...
      if (retval) destroyFolderCache(getFolderCache(folder.getFullName()));
      return retval;
  }
  ...
  public String renameFolder(String orig, String newname) throws MessagingException {
      ...
      boolean done=oldfolder.renameTo(newfolder);
      if (!done) throw new MessagingException("Rename failed");
  ...
  public Exception sendMsg(String from, SimpleMessage smsg, ArrayList<Attachment> attachments) {
    ...
    if (ident!=null && ident.mainfolder!=null && ident.mainfolder.trim().length()>0) {
	  ...
	  try {
		...
		if (folder.exists()) sentfolder=newsentfolder;
	  } catch(MessagingException exc) {
...
//        System.out.println("to["+q+"]="+to[q]);
    ...
    for(int i=0;i<headerLines.length;++i) {
        if (!headerLines[i].startsWith("Sonicle-reply-folder")) msg.addHeaderLine(headerLines[i]);
    }
    ...
    if (references!=null && references[0]!=null) msg.setHeader("References", references[0]);
    if (tosave) {
        if (replyfolder!=null) msg.setHeader("Sonicle-reply-folder", replyfolder);
    ...
    if (forwardedfrom!=null) msg.setHeader("Forwarded-From", forwardedfrom);
    if (tosave) {
        if (forwardedfolder!=null) msg.setHeader("Sonicle-forwarded-folder", forwardedfolder);
        msg.setHeader("Sonicle-draft", "true");
    }
    //set the subject
...
//              System.out.println("adding attachment mime "+contentType+" filename "+contentFileName);
      ...
      if(newAttach>0) { //if there are new attachments
        ...
        for(int e=0; e<newAttach; e++) {
          ...
          //
          if (attach.isInline()) mbps[e].setDisposition(Part.INLINE);
...
/*        String mid=m.getHeader("Message-ID")[0];
  ...
  private SimpleMessage getForwardMsg(int id, Message msg, boolean richContent, String fromtitle, String totitle, String cctitle, String datetitle, String subjecttitle, boolean attached) {
    ...
    try {
        ...
        if (vh!=null) msgid=vh[0];
        if (msgid!=null) forward.setHeader("Forwarded-From", msgid);
    } catch(MessagingException exc) {
        exc.printStackTrace();
    }
...
//      System.out.println("*** SimpleMessage: " +e);
  ...
  private String getForwardBody(Message msg, String body, int format, boolean isHtml, String fromtitle, String totitle, String cctitle, String datetitle, String subjecttitle) throws MessagingException {
    ...
    String msgDate="";
    if (dt!=null) msgDate=DateFormat.getDateTimeInstance(java.text.DateFormat.LONG,java.text.DateFormat.LONG, locale).format(dt);
    ...
    String cr="\n";
    if (format!=SimpleMessage.FORMAT_TEXT) cr="<BR>";
    if (format!=SimpleMessage.FORMAT_HTML) {
      if (format==SimpleMessage.FORMAT_PREFORMATTED) sb.append("<TT>");
      sb.append(cr+cr+cr+"----------------------------------------------------------------------------------"+cr+cr);
      sb.append(fromtitle+": "+msgFrom+cr);
      if (msgTo!=null) sb.append(totitle+": "+msgTo+cr);
      if (msgCc!=null) sb.append(cctitle+": "+msgCc+cr);
      ...
      if (msgTo!=null) sb.append("<B>"+totitle+":</B> "+msgTo+"<BR>");
      if (msgCc!=null) sb.append("<B>"+cctitle+":</B> "+msgCc+"<BR>");
...
//        sb.append("</td></tr></table>");
  ...
  private String getTextPlainContentAsString(Part p) throws IOException, MessagingException {
        ...
        if (!java.nio.charset.Charset.isSupported(charset)) charset="ISO-8859-1";
        try {
        ...
        if (istream==null) throw new IOException("Unknown message class "+p.getClass().getName());
  ...
  private boolean appendReplyParts(Part p, StringBuffer htmlsb, StringBuffer textsb, ArrayList<String> attnames) throws MessagingException,
    ...
    if (disp!=null && disp.equalsIgnoreCase(Part.ATTACHMENT)) return false;
    if(p.isMimeType("text/html")) {
      ...
      if(appendReplyParts((MimeMessage)content, htmlsb, textsb,attnames)) {
        isHtml=true;
          ...
          if (filename!=null && attnames!=null) attnames.add(filename);
    }
    textsb.append('\n');
    textsb.append('\n');
  ...
  public void cleanup() {
    if (mft!=null) mft.abort();
    if (ast!=null && ast.isRunning()) ast.cancel();
...
/*    public void updateServerEvents() {
    ...
    private int[] toInts(String values[]) {
        ...
        for(int i=0;i<values.length;++i) ret[i]=Integer.parseInt(values[i]);
        return ret;
    }
  ...
  public boolean isSentFolder(String fullname) {
    ...
    if (lastname.equals(plastname)) return true;
    return false;
  }
  ...
  public boolean isInboxFolder(String foldername) {
    if (foldername.equals("INBOX")) return true;
    return false;
  }
  ...
  public boolean isDocMgtFolder(String foldername) {
      UserProfile profile=environment.getProfile();
      if (!profile.hasDocumentManagement()) return false;
      boolean b=false;
  ...
  public String getInternationalFolderName(FolderCache fc) {
    ...
    if (fc.isSharedFolder()) desc=lookupResource(MailLocaleKey.FOLDERS_SHARED);
    else if (fc.isInbox()) desc=lookupResource(MailLocaleKey.FOLDERS_INBOX);
    else if (fc.isDrafts()) desc=lookupResource(MailLocaleKey.FOLDERS_DRAFTS);
    else if (fc.isTrash()) desc=lookupResource(MailLocaleKey.FOLDERS_TRASH);
    else if (fc.isSent()) desc=lookupResource(MailLocaleKey.FOLDERS_SENT);
    else if (fc.isSpam()) desc=lookupResource(MailLocaleKey.FOLDERS_SPAM);
    return desc;
  }
  ...
  private void checkCreateFolder(String foldername) throws MessagingException {
      ...
      if (!folder.exists()) folder.create(Folder.HOLDS_MESSAGES|Folder.HOLDS_FOLDERS);
  }
...
/*    attachments=getCloudAttachments(msgid);
  ...
  private FolderCache createFolderCache(Folder f) throws MessagingException {
	...
	private boolean isLeaf(IMAPFolder folder) throws MessagingException {
        ...
        for(String att: atts) {
            if (att.equals("\\HasChildren")) leaf=false;
    ...
    protected FolderCache addFoldersCache(FolderCache parent, Folder child) throws MessagingException {
        ...
        if (!leaf) _loadFoldersCache(fcChild);
        return fcChild;
    }
    ...
    private void destroyFolderCache(FolderCache fc) {
        ...
        if (fcc!=null) {
            ...
            if (len>0) {
                ...
                for(FolderCache child: afc) destroyFolderCache(child);
            }
        }
        FolderCache fcp=fc.getParent();
        if (fcp!=null) fcp.removeChild(fc);
        fc.cleanup();
        try { fc.close(); } catch(Exception exc) {}
    }
    ...
    public void processGetImapTree(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            checkStoreConnected();
            if (pfoldername.equals("root")) folder=getDefaultFolder();
    ...
    private void outputFolders(Folder parent, Folder folders[], boolean level1, PrintWriter out) throws Exception {
        ...
        if (isSharedFolder(parent.getFullName())) {
            ...
            if (ss.equals("D")) Collections.sort(afolders,dcomp);
            else if (ss.equals("U")) Collections.sort(afolders,ucomp);
        }
        ...
        for(Folder f: afolders) {
            ...
            if (mc.isSharedFolder()) { iconCls="iconSharedFolder"; nounread=true; }
            else if (mc.isInbox()) iconCls="iconInboxFolder";
            else if (mc.isSent()) { iconCls="iconSentFolder"; nounread=true; }
            else if (mc.isDrafts()) { iconCls="iconDraftsFolder"; nounread=true; }
            else if (mc.isTrash()) { iconCls="iconTrashFolder"; nounread=true; }
            else if (mc.isSpam()) { iconCls="iconSpamFolder"; nounread=true; }
            else if (mc.isDocMgt()) iconCls="iconDocMgtFolder";
            else if (mc.isSharedInbox()) { iconCls="iconInboxFolder"; }
            if (!nounread) {
            ...
            String ss="{id:'"+OldUtils.jsEscape(foldername)+
                    ...
                    "',unread:"+unread+
                    ",hasUnread:"+hasUnread;
            if (mc.isInbox()) ss+=",isInbox: true";
            if (mc.isSent()) ss+=",isSent: true";
            if (mc.isDrafts()) ss+=",isDrafts: true";
            if (mc.isTrash()) ss+=",isTrash: true";
            if (mc.isSpam()) ss+=",isSpam: true";
            if (mc.isScanForcedOff()) ss+=", scanOff: true";
            else if (mc.isScanForcedOn()) ss+=", scanOn: true";
            else if (mc.isScanEnabled()) ss+=", scanEnabled: true";
            ss+="},";
            out.print(ss);
    ...
    private ArrayList<Folder> sortFolders(Folder folders[]) {
        ...
        if (trash!=null) afolders.add(0,trash);
        if (spam!=null) afolders.add(0,spam);
        if (sent!=null) afolders.add(0,sent);
        if (drafts!=null) afolders.add(0,drafts);
        if (inbox!=null) afolders.add(0,inbox);
        return afolders;
    }
    ...
    public void processMoveMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (allfiltered==null) {
                ...
                if (!mf) moveMessages(mcache, tomcache, toInts(ids));
                else {
                    int iids[]=new int[1];
                    for(String id: ids) {
                        int ix=id.indexOf("|");
    ...
    public void processCopyMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (allfiltered==null) {
                ids=request.getParameterValues("ids");
                if (!mf) copyMessages(mcache, tomcache, toInts(ids));
                else {
                    int iids[]=new int[1];
                    for(String id: ids) {
                        int ix=id.indexOf("|");
    ...
    public void processArchiveMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            FolderCache mcache=getFolderCache(fromfolder);
            ids=request.getParameterValues("ids");
            if (!mf) archiveMessages(mcache, toInts(ids), idcategory, idsubcategory);
            else {
                int iids[]=new int[1];
                for(String id: ids) {
                    ix=id.indexOf("|");
    ...
    public void processArchiveMessagesWt(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            FolderCache mcache=getFolderCache(fromfolder);
            ids=request.getParameterValues("ids");
            if (!mf) archiveMessagesWt(mcache, toInts(ids), idcategory, idsubcategory,customer_id);
            else {
                int iids[]=new int[1];
                for(String id: ids) {
                    ix=id.indexOf("/");
    ...
    String[] getMessageIds(FolderCache mcache,HttpServletRequest request) throws MessagingException {
        ...
        if (psearchfield!=null && psearchfield.trim().length()==0) psearchfield=null;
        if (ppattern!=null && ppattern.trim().length()==0) ppattern=null;
        if (psortfield==null) psortfield="date";
        if (psortdir==null) psortdir="DESC";
        int sortby=0;
        if (psortfield.equals("messageid")) sortby=MessageComparator.SORT_BY_MSGIDX;
        else if (psortfield.equals("date")) sortby=MessageComparator.SORT_BY_DATE;
        else if (psortfield.equals("priority")) sortby=MessageComparator.SORT_BY_PRIORITY;
        else if (psortfield.equals("to")) sortby=MessageComparator.SORT_BY_RCPT;
        else if (psortfield.equals("from")) sortby=MessageComparator.SORT_BY_SENDER;
        else if (psortfield.equals("size")) sortby=MessageComparator.SORT_BY_SIZE;
        else if (psortfield.equals("subject")) sortby=MessageComparator.SORT_BY_SUBJECT;
        else if (psortfield.equals("status")) sortby=MessageComparator.SORT_BY_STATUS;
        else if (psortfield.equals("flag")) sortby=MessageComparator.SORT_BY_FLAG;
        ...
        if (group.equals("messageid")) sort_group=MessageComparator.SORT_BY_MSGIDX;
        else if (group.equals("gdate")) { sort_group=MessageComparator.SORT_BY_DATE; groupascending=false; }
        else if (group.equals("priority")) sort_group=MessageComparator.SORT_BY_PRIORITY;
        else if (group.equals("to")) sort_group=MessageComparator.SORT_BY_RCPT;
        else if (group.equals("from")) sort_group=MessageComparator.SORT_BY_SENDER;
        else if (group.equals("size")) sort_group=MessageComparator.SORT_BY_SIZE;
        else if (group.equals("subject")) sort_group=MessageComparator.SORT_BY_SUBJECT;
        else if (group.equals("status")) sort_group=MessageComparator.SORT_BY_STATUS;
        else if (group.equals("flag")) sort_group=MessageComparator.SORT_BY_FLAG;
...
/*            String xids[];
    ...
    public void processDeleteMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            checkStoreConnected();
            FolderCache mcache=getFolderCache(fromfolder);
            if (!mf) deleteMessages(mcache, toInts(ids));
            else {
                int iids[]=new int[1];
                for(String id: ids) {
                    int ix=id.indexOf("|");
    ...
    public void processFlagMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            FolderCache mcache=getFolderCache(fromfolder);
            if (!mf) {
                if (flag.equals("clear")) clearMessagesFlag(mcache, toInts(ids));
                else flagMessages(mcache, toInts(ids), flag);
            } else {
                int iids[]=new int[1];
                for(String id: ids) {
                    int ix=id.indexOf("|");
                    ...
                    mcache=getFolderCache(fromfolder);
                    iids[0]=Integer.parseInt(id);
                    if (flag.equals("clear")) clearMessagesFlag(mcache, iids);
                    else flagMessages(mcache, iids, flag);
                }
            }
            sout="{\nresult: true\n}";
        } catch(MessagingException exc) {
    ...
    public void processSeenMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            checkStoreConnected();
            FolderCache mcache=getFolderCache(fromfolder);
            if (!mf) setMessagesSeen(mcache, toInts(ids));
            else {
                int iids[]=new int[1];
                for(String id: ids) {
                    int ix=id.indexOf("|");
    ...
    public void processUnseenMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            checkStoreConnected();
            FolderCache mcache=getFolderCache(fromfolder);
            if (!mf) setMessagesUnseen(mcache, toInts(ids));
            else {
                int iids[]=new int[1];
                for(String id: ids) {
                    int ix=id.indexOf("|");
    ...
    public void processSetScanFolder(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (svalue!=null) value=svalue.equals("1");
        boolean recursive=false;
        String srecursive=request.getParameter("recursive");
        if (srecursive!=null) recursive=srecursive.equals("1");
        String sout=null;
        Connection con=null;
        Statement stmt=null;
        try {
    ...
    private void setMessagesSeen(FolderCache mcache, boolean seen, boolean recursive) throws MessagingException {
        if (seen) mcache.setMessagesSeen();
        else mcache.setMessagesUnseen();
        if (recursive) {
            ArrayList<FolderCache> children=mcache.getChildren();
            if (children!=null) {
                for(FolderCache fc: children) setMessagesSeen(fc,seen,recursive);
            }
        }
    }
    ...
    public String getPartName(Part p) throws MessagingException {
        ...
        if (pname==null) {
            ...
            if (hctypes==null || hctypes.length==0) return null;
            String hctype=hctypes[0];
            int ix=hctype.indexOf("name=");
            if (ix>=0) {
                int sx=ix+5;
                int ex=hctype.indexOf(";",sx);
                if (ex>=0) pname=hctype.substring(sx,ex);
                else pname=hctype.substring(sx);
                pname=pname.trim();
        ...
        try { pname=OldUtils.decodeQString(pname,"iso-8859-1"); } catch(Exception exc) { exc.printStackTrace();}
        return pname;
    }
...
/*    public void processSavePaneSize(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
    ...
    public void processNewFolder(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (folder==null) mcache=fcRoot;
            else mcache=getFolderCache(folder);
            newfolder=mcache.createFolder(name);
            if (newfolder==null) result=false;
            else {
                if (mcache!=fcRoot) sout+="parent: '"+OldUtils.jsEscape(mcache.getFolderName())+"',\n";
    ...
    private String normalizeName(String name) throws MessagingException {
        ...
        while(name.contains(sep)) name=name.replace(sep, "_");
        return name;
    }
    ...
    public void processDeleteFolder(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            sout="{\n";
            mcache=getFolderCache(folder);
            if (isSpecialFolder(folder)) result=false;
            else {
    ...
    public void processMoveFolder(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (isSpecialFolder(folder)) result=false;
                ...
                sout+="newid: '"+OldUtils.jsEscape(newf.getFullName())+"',\n";
                sout+="newname: '"+OldUtils.jsEscape(newf.getName())+"',\n";
                if (to!=null) sout+="parent: '"+OldUtils.jsEscape(newf.getParent().getFullName())+"',\n";
                result=true;
            }
            sout+="result: "+result+"\n}";
        } catch(MessagingException exc) {
    ...
    public void processGetSource(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (!headers) {
                ...
                String line=null;
                while((line=br.readLine())!=null) sb.append(OldUtils.htmlescape(line)+"\n");
            }
            sb.append("</pre>");
    ...
    public void processGetReplyMessage(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (preplyall!=null && preplyall.equals("1")) replyAll=true;
        String sout=null;
        try {
            checkStoreConnected();
            FolderCache mcache=getFolderCache(pfoldername);
            Message m=mcache.getMessage(Integer.parseInt(pidmessage));
            if (m.isExpunged()) throw new MessagingException("Message "+pidmessage+" expunged");
            ...
            if (!attached) {
                ...
                for (int i = 0; i < maildata.getAttachmentPartCount(); ++i) {
                 ...
                 if (filename != null) {
                   ...
                   String cid = null;
                   //String cid=filename;
                   if (cids != null && cids[0] != null) cid = cids[0];
    ...
    public void processGetEditMessage(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (isDraftsFolder(pfoldername)) {
                if (vheader!=null && vheader[0]!=null) receipt=true;
            ...
            if (vs!=null && vs[0]!=null) inreplyto=vs[0];
            references=m.getHeader("References");
            if (inreplyto!=null) {
                vs=m.getHeader("Sonicle-reply-folder");
                String replyfolder=null;
                if (vs!=null && vs[0]!=null) replyfolder=vs[0];
                if (replyfolder!=null) sout+=" replyfolder: '"+OldUtils.jsEscape(replyfolder)+"',";
                sout+=" inreplyto: '"+OldUtils.jsEscape(inreplyto)+"',";
            }
            if (references!=null) {
                String refs="";
            ...
            if (vs!=null && vs[0]!=null) forwardedfrom=vs[0];
            if (forwardedfrom!=null) {
                vs=m.getHeader("Sonicle-forwarded-folder");
                String forwardedfolder=null;
                if (vs!=null && vs[0]!=null) forwardedfolder=vs[0];
                if (forwardedfolder!=null) sout+=" forwardedfolder: '"+OldUtils.jsEscape(forwardedfolder)+"',";
                sout+=" forwardedfrom: '"+OldUtils.jsEscape(forwardedfrom)+"',";
            }
...
/*        String subject=request.getParameter("subject");
        ...
        try {
            ...
            if (ifrom!=null) from=ifrom.email;
            checkStoreConnected();
...
/*                if (vfs!=null && hashlinks!=null && hashlinks.size()>0) {
                ...
                sout="{\nresult: true,";
                if (foundfolder!=null) sout+=" foundfolder: '"+OldUtils.jsEscape(foundfolder)+"',";
                sout+=" saved: false\n}";
            }
            else {
    ...
    private String flagAnsweredMessage(String replyfolder, String id) throws MessagingException {
        String foundfolder=null;
        if (replyfolder!=null) {
            if (_flagAnsweredMessage(replyfolder,id)) foundfolder=replyfolder;
        }
        if (foundfolder==null) {
    ...
    private String flagForwardedMessage(String forwardedfolder, String id) throws MessagingException {
        ...
        if (forwardedfolder!=null) {
            if (_flagForwardedMessage(forwardedfolder,id)) foundfolder=forwardedfolder;
        }
        if (foundfolder==null) {
            SonicleIMAPFolder f=(SonicleIMAPFolder)store.getFolder("");
    ...
    public void processSaveMessage(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            checkStoreConnected();
            FolderCache fc=null;
            if (savefolder==null) fc=getFolderCache(mprofile.getFolderDrafts());
            else fc=getFolderCache(savefolder);
        ...
        for(int i=0;i<emails.length;++i) {
          ...
          if (email.indexOf('@')<0) {
            if (isFax && OldUtils.isNumeric(email)) {
                ...
                if (ix0<0) name=email;
                else name=email.substring(0,ix0).trim();
                String dmid=null;
                ...
                else dmid="privatedb: "+profile.getUserId();
...
/*                ContactsService cs=(ContactsService)wts.getServiceByName("contacts");
        ...
        Identity from=null;
        if (idx>=0) from=mprofile.getIdentity(idx);
        msg.setFrom(from);
        msg.setTo(to);
        msg.setCc(cc);
        msg.setBcc(bcc);
        ...
        if (isFax) {
            ...
            if (coverpage!=null) {
                if (coverpage.equals("none")) msg.addHeaderLine("X-FAX-AutoCoverPage: No");
                else {
		...
		if(headersKeys!=null && headersValues!=null && headersKeys.length == headersValues.length) {
			for(int i=0; i<headersKeys.length; i++) {
				if(!headersKeys[i].equals("")) msg.addHeaderLine(headersKeys[i] + ": " + headersValues[i]);
			}
		}

        if (inreplyto!=null) msg.setInReplyTo(inreplyto);
        if (references!=null) msg.setReferences(new String[] { references });
        if (replyfolder!=null) msg.setReplyFolder(replyfolder);

        if (forwardedfolder!=null) msg.setForwardedFolder(forwardedfolder);
        if (forwardedfrom!=null) msg.setForwardedFrom(forwardedfrom);
...
/*                String surl=webtopapp.getUri();
          ...
          else msg.setContent(content,null,mime);
    ...
    private void checkEmail(String email) throws AddressException {
        int ix=email.indexOf('@');
        if (ix<1) throw new AddressException(email);
        ...
        if (ix2>=0) {
            ...
            else throw new AddressException(email);
        }
    }
...
/*    public void processUploadCloudAttachments(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
    ...
    public void processSendReceipt(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (ident!=null) from=ident.displayname+" <"+ident.email+">";
...
/*	public void processGroupChanged(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
    ...
    public void processListMessages(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (pstart!=null) start=Integer.parseInt(pstart);
        if (plimit!=null) limit=Integer.parseInt(plimit);
        String sout="{\n";
        Folder folder=null;
        try {
            checkStoreConnected();
            int funread=0;
            if (pfoldername==null) folder=getDefaultFolder();
            else folder=getFolder(pfoldername);
            ...
            if (ppattern!=null && psearchfield!=null) key+="|"+ppattern+"|"+psearchfield;
            ...
            synchronized(mlt.lock) {
                ...
                if (!mlt.finished) {
                    try { mlt.lock.wait(); } catch(InterruptedException exc) { exc.printStackTrace(); }
                    mlThreads.remove(key);
                }
            }
            Message msgs[]=null;
            if (mlt.lastRequest==millis) msgs=mlt.msgs;
...
/*               if (ppattern==null && !isSpecialFolder(mcache.getFolderName())) {
                ...
                for(int i=0,ni=0;i<limit;++ni,++i) {
                    ...
                    if (d!=null) {
                        ...
                        if (cal1.get(Calendar.MONTH)==cal2.get(Calendar.MONTH) && cal1.get(Calendar.YEAR)==cal2.get(Calendar.YEAR)) {
                            ...
                            if (dx==0) {
                                isToday=true;
                                if (isGdate) gdate=environment.lookupCoreResource(locale,CoreLocaleKey.DATE_TODAY)+"  "+gdate;
                            }
                            else if (dx==1 && isGdate) gdate=environment.lookupCoreResource(locale, CoreLocaleKey.DATE_YESTERDAY)+"  "+gdate;
                        }
                    }
    ...
    //                    if (flags.contains(Flags.Flag.USER)) flagImage=webtopapp.getUri()+"/images/themes/"+profile.getTheme()+"/mail/flag.gif";
                    ...
                    //User flags
                    String cflag="";
                    for (String flagstring: MailService.flagStrings) {
                    ...
                    if (profile.hasDocumentManagement()) archived=m.getHeader("X-WT-Archived")!=null;
                    sout+="{idmessage:'"+nid+"',"+
                            "priority:"+priority+","+
                            "status:'"+status+"',"+
                            "to:'"+to+"',"+
...
/*                if (ppattern==null && !isSpecialFolder(mcache.getFolderName())) {
                ...
                if (!mcache.isSpecial()) mcache.refreshUnreads();
                funread=mcache.getUnreadMessagesCount();
                sout+="\n],\n";
                sout+="metaData: {\n"+
...
/*                WebTopService provider=wts.getServiceByName(providername);
            ...
            if (as!=null && as.length>0) {
                ...
                if (fromName==null) fromName=fromEmail;
            }
            ...
            if (ccs!=null)
                for(Address cc: ccs) {
                    ...
                    if (ccName==null) ccName=ccEmail;
                    sout+="{iddata:'cc',value1:'"+OldUtils.jsEscape(ccName)+"',value2:'"+OldUtils.jsEscape(ccEmail)+"',value3:0},\n";
                    ++recs;
                }
            ArrayList<String> htmlparts=null;
            if (providername==null) htmlparts=mcache.getHTMLParts((MimeMessage)m,msgnum,false);
            else htmlparts=mcache.getHTMLParts((MimeMessage)m, providername, providerid);
            ...
            for(int i=0;i<acount;++i) {
                ...
                if (p.getDisposition()!=null && p.getDisposition().equalsIgnoreCase(Part.INLINE) && inline) continue;
                ...
                if (pname==null) {
                    ix=ctype.indexOf("/");
                    String fname=ctype;
                    if (ix>0) fname=ctype.substring(ix+1);
                    String ext=environment.getExtension(ctype);
                    if (ext==null) pname=fname;
                    else pname=fname+"."+ext;
                }
                int size=p.getSize();
                int lines=(size/76);
            ...
            if (providername==null && !mcache.isSpecial()) mcache.refreshUnreads();
            long millis=System.currentTimeMillis();
            sout+="\n],\ntotalRecords:"+recs+",\nmillis:"+millis+"\n}\n";
            out.println(sout);
//            if (!wasopen) folder.close(false);
    ...
    public void processSaveMessageNote(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            else msg.setFlags(flagNote, false);
            result=true;
        } catch(Exception exc) {
            exc.printStackTrace();
            message=exc.getMessage();
        } finally {
            if (pstmt!=null) try { pstmt.close(); } catch(Exception exc) {}
            if (stmt!=null) try { stmt.close(); } catch(Exception exc) {}
            if (con!=null) try { con.close(); } catch(Exception exc) {}
        }
...
//            sout+=" { email: \""+OldUtils.jsEscape(OldUtils.htmlescape(cemail))+"\" }";
    ...
    public void processGetAttachment(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            HTMLMailData mailData=mcache.getMailData((MimeMessage)m);
            Part part=null;
            if (pcid!=null) part=mailData.getCidPart(pcid);
            else if (purl!=null) part=mailData.getUrlPart(purl);
            else if (pidattach!=null) part=mailData.getAttachmentPart(Integer.parseInt(pidattach));
            else if (punknown!=null) part=mailData.getUnknownPart(Integer.parseInt(punknown));
...
/*            byte[] b = new byte[64 * 1024];
    ...
    public void processPreviewAttachment(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            Attachment att=null;
            if (tempname!=null) att=getAttachment(msgid,tempname);
            else if (cid!=null) att=getAttachmentByCid(msgid, cid);
            if (att != null) {
...
/*    public void processEditFilter(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
    ...
    public void processPollAdvancedSearch(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        try {
            ...
            int start=0;
            if (sstart!=null) start=Integer.parseInt(sstart);
            String sout="{\n";
            if (ast!=null) {
                UserProfile profile=environment.getProfile();
                Locale locale=profile.getLocale();
    ...
    //                    if (flags.contains(Flags.Flag.USER)) flagImage=webtopapp.getUri()+"/images/themes/"+profile.getTheme()+"/mail/flag.gif";
                    ...
                    boolean flagComplete=flags.contains("complete");
                    if (flagComplete) cflag+="-complete";
	...
	public MailFilters getMailFilters(Connection con, String filtersTable, String login, String iddomain) throws Exception {
		...
		try {
			...
			while(rs.next()) {
			  ...
			  String operator="or";
			  if (rs.getString("condition").equals("ALL")) operator="and";
			  ...
			  if (size!=null && size.trim().length()>0 && sizevalue!=null && sizevalue.trim().length()>0) {
				MailFilterCondition mfc=new MailFilterCondition();
				if (size.equals("+")) mfc.setComparison(MailFilterCondition.GREATERTHAN);
				else mfc.setComparison(MailFilterCondition.LESSTHAN);
				mfc.setField("size");
			...
			if (rs.next()) {
			  ...
			  if (vactive) filters.setVacation(message, addresses);
			}
			rs.close();
			stmt.close();
			...
			if (rs!=null) try { rs.close(); } catch(SQLException exc2) {}
			if (stmt!=null) try { stmt.close(); } catch(SQLException exc2) {}
		}
...
/*        Connection con=null;
        ...
        Connection con=null;
        try {
            con=environment.getCoreConnection();
